...
def bootstrapping_todas_emociones(emociones_total_PCA_normalizado, emociones_total_LR_normalizado, emociones, iteraciones=1000, verbose=True):

    for emocion in emociones:

        promedio_emocion, promedios_aleatorios = evaluar_similitud(emociones_total_PCA_normalizado, emocion, iteraciones)

        optional_print(f'Promedio umbralizado para {emocion} (PCA): {promedio_emocion}', verbose)
        optional_print(f"Promedio de los promedios aleatorios (PCA): {mean(promedios_aleatorios)}", verbose)

        promedio_emocion, promedios_aleatorios = evaluar_similitud(emociones_total_LR_normalizado, emocion, iteraciones)

        optional_print(f'Promedio umbralizado para {emocion} (LR): {promedio_emocion}', verbose)
        optional_print(f"Promedio de los promedios aleatorios (LR): {mean(promedios_aleatorios)}", verbose)

def umbrales_optimos(df, iteraciones=100, verbose=True):
    ...
    optional_print(umbrales_optimos, verbose)
    return resultados