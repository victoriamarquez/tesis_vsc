...
# Directory for Torch extensions cache
TORCH_EXTENSIONS_DIR="/scratch/torch_extensions_cache"
...
if [[ "${CONTAINER_ID}" ]]; then
    docker run --shm-size=2g --gpus all -it --rm -v `pwd`:/scratch --user $(id -u):$(id -g) \
        --workdir=/scratch -e HOME=/scratch -e TORCH_EXTENSIONS_DIR=${TORCH_EXTENSIONS_DIR} $IMAGE $@
else
